function [b, kc, ka] = newtonStepCLVF_heur(b0, kc0, ka0, alpha, amax, wmax, wandamax,rC_T0, vC_T0, W_CLVF, mu, tol, gamma, beta, r_factor, W_s)
    ...
    while 1 % Go on forever.
        ...
        % First, check gradient:
            [dCdp, dSdp0] = costGradientCLVF_heur(alpha,b0,ka0,kc0, ...
                                                W_CLVF,wmax,wandamax,rC_T0, vC_T0, s0, mu, r_factor, W_s);
        ...
        if stdGrad <= tol % We ARE good enough.
            ...
            % Compute Hessian, the compute step.
                dC2dp2 = costHessianCLVF_heur(alpha,b0,ka0,kc0,rC_T0,vC_T0,wandamax, W_CLVF, s0, dSdp0,wmax, mu, r_factor, W_s);
                xStep = -dC2dp2\dCdp;
