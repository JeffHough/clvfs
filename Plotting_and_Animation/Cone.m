 function [Cone,EndPlate1,EndPlate2] = Cone(X1,X2,R,n,cyl_color,closed,lines, varargin)
%
...
if closed==1
    hold on
    EndPlate1=fill3(xx1(:,1),xx2(:,1),xx3(:,1),'r',varargin{:});
    EndPlate2=fill3(xx1(:,2),xx2(:,2),xx3(:,2),'r',varargin{:});
end

% Plotting the cylinder along the X-Direction with required length starting
% from Origin
Cone=mesh(xx1,xx2,xx3,varargin{:});
...
set(Cone,'AmbientStrength',1,'FaceColor',cyl_color,'FaceLighting','gouraud','EdgeColor','k');%,'EdgeColor','none') %MADE ADJUSTMENT HERE
if closed==1
