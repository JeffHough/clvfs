function [b, kc, ka] = newtonStepCLVF_quad(b0, kc0, ka0, alpha, amax, wmax, wandamax, W_CLVF, mu, tol, gamma, beta)
...
%         C0 = costFunctionCLVF_quad(b0,kc0, ka0, s0, W_CLVF, mu);
    ...
    while 1 % Go on forever.
        ...
        % First, check gradient:
            [dCdp, dSdp0] = costGradientCLVF_quad(alpha,b0,ka0,kc0, ...
                                                W_CLVF,wmax, s0, mu);
        ...
        if stdGrad <= tol % We ARE good enough!!
            ...
            % Compute Hessian, the compute step.
                dC2dp2 = costHessianCLVF_quad(alpha,b0,ka0,kc0, W_CLVF, s0, dSdp0,wmax, mu);
                xStep = -dC2dp2\dCdp;
